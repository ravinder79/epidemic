...
person["velocity"] = (-0.5 + np.random.random((n, 2))) * 30
...
def update(frame_number):
    ...
    for c in cl:
        if person["trip"][c] == 1 and person["counter"][c] <= 8:
        ...
        if person["counter"][c] > 8 and (
            (abs(person["position"][c][0]) < abs(po[np.where(cl == c)[0][0]][0]))
            or (abs(person["position"][c][1]) < abs(po[np.where(cl == c)[0][0]][1]))
        ):
            person["position"][c][0] = hop.hopr(
                po[np.where(cl == c)[0][0]][0], person["position"][c][0]
            )
            person["position"][c][1] = hop.hopr(
                po[np.where(cl == c)[0][0]][1], person["position"][c][1]
            )

        if (
            (person["counter"][c] > 8)
            and (
                abs(person["position"][c][0]) - abs(po[np.where(cl == c)[0][0]][0]) >= 0
            )
            and (
                abs(person["position"][c][1]) - abs(po[np.where(cl == c)[0][0]][1]) >= 0
            )
        ):
            person["trip"][c] = 0
            person["counter"][c] = 0
...
# director.
anim = animation.FuncAnimation(fig, update, interval=10, frames=1600)

# save command below
# anim.save('c_q0_sd0.mp4', fps = 15.0, dpi=500)
plt.show()
